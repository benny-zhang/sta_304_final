...
# Authors: Rohan Alexander, Sam Caetano, Benjamin Zhang
...
library(janitor)
library(tidyverse)
setwd("C:/Users/longh/OneDrive/Documents/STA304/Final_assignment_Sta304/GSS_raw_data")
...
##Start of Benjamin Zhang's contribution to this code##

#The relavant data we need to post-stratify the survey data
select_survey <- gss %>% 
  select(province,sex,age,education) %>%
  drop_na(education,province,age,sex)

#this GSS data was taken from 2017 and we have to account for the 2 years before the 2019 election. 
select_survey$age_range <- floor(select_survey$age) + 2
select_survey <- select_survey %>%
  filter(age >= 18)

#Generating an age range so we can stratify our population into distinct groups
select_survey$age_range[select_survey$age_range %in% 18:29] <- "18-29"
select_survey$age_range[select_survey$age_range %in% 30:39] <- "30-39"
select_survey$age_range[select_survey$age_range %in% 40:49] <- "40-49"
select_survey$age_range[select_survey$age_range %in% 50:59] <- "50-59"
select_survey$age_range[select_survey$age_range %in% 60:69] <- "60-69"
select_survey$age_range[select_survey$age_range %in% 70:79] <- "70-79"
select_survey$age_range[select_survey$age_range %in% 80:89] <- "80-89"
select_survey$age_range[select_survey$age_range %in% 90:99] <- "90-99"

Select_Survey<- select_survey

#drop the 'age' coloumn 
drops <- c("age")
Select_Survey[ , !(names(Select_Survey) %in% drops)]


#Seperate education values into four categories, "No Highschool", "Highschool", "Undergraduate", "Graduate+"

Select_Survey$education[Select_Survey$education == "Less than high school diploma or its equivalent" ] <- "No Highschool"

Select_Survey$education[Select_Survey$education == "High school diploma or a high school equivalency certificate" ] <- "Highschool"
Select_Survey$education[Select_Survey$education == "College, CEGEP or other non-university certificate or di..." ] <- "Highschool"
Select_Survey$education[Select_Survey$education == "Trade certificate or diploma" ] <- "Highschool"
Select_Survey$education[Select_Survey$education == "University certificate or diploma below the bachelor's level" ] <- "Highschool"

Select_Survey$education[Select_Survey$education == "Bachelor's degree (e.g. B.A., B.Sc., LL.B.)" ] <- "Undergraduate"

Select_Survey$education[Select_Survey$education == "University certificate, diploma or degree above the bach..." ] <- "Graduate+"

Select_Survey1 <- Select_Survey %>%
  select(education)

Select_Survey1 <- Select_Survey1 %>%
  count(education)

#count the amount of individuals in each group, new column value 'n' holds count
cgss<- Select_Survey %>%
  count(province, sex, age_range, education)



#write dataframe as csv for use in RMD paper 
write_csv(cgss, "../ProcessedData/select_gss.csv")
